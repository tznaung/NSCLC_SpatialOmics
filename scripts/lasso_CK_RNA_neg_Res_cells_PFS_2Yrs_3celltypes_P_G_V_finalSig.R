...
cell_genes1 = read.csv("Malignant_cell_genes.csv")
cell_genes2 = read.csv("Granulocytes_cell_genes.csv")
cell_genes3 = read.csv("Malignant_cell_genes_ChinWee.csv")
cell_genes4 = read.csv("Granulocytes_cell_genes_ChinWee.csv")
cell_genes5 = read.csv("Endothelial_cell_genes.csv")
...
CK = read.csv("UQ_CK_large_PFS_OS_only_1st_2nd_line.csv", row.names = 2)
cell_genes = intersect(cell_genes, colnames(CK))

# get Sig DE genes for training set 

CK_df = read.csv("CK_av_3B.csv", header = T)
CK_df$Spot_ID <- CK_df$X
Res_CK = read.csv("Yale_firstLine_pre_patients.csv", header = T)
CK_2 = merge(CK_df, Res_CK, by = "Spot_ID", all = FALSE)
CK_2$group <- CK_2$PFS_5Years_Index # define the group you want to compare
data = CK_2[,3:10134]
prop_test = 0.20
...
idx0 = which(CK_2$group==1) 
idx1 = which(CK_2$group==0)

hzrs = read.csv("hzrs_CK_PFS_5Years_3celltypes_resistance_P_G_V_CW_genes.csv")

#good_models = which((hzrs$x)<=0.7 & (hzrs$x)>0)
#good_models = which((hzrs$x)<=0.3)
...
for (cSplit in good_models){
  ...
  #train_idx = setdiff(seq_len(nrow(CK_2)), test_idx)
  train_data = CK_2[train_idx,]
  train_data2 = train_data[,3:10134]
  test_data = CK_2[test_idx,]
  test_data2 = test_data[,3:10134]
  ...
  #write.csv(response$table, "CK_train_set_Sig_Genes.csv")
  ...
  if (do_lasso){
    ...
    for(cSeed in 1:nSeeds){
      ...
      model_cv <- glmnet(as.matrix(train_data3[,1:nGene]), y_df, alpha = 0.1, lambda = lambda_cv, standardize = FALSE, family = "cox", lower.limits = 0) ###
    ...
    if (length(nonzero)>1){
      ...
      model_cv <- glmnet(as.matrix(train_data3[,nonzero]), y_df, alpha = 0.1, lambda = 0, standardize = FALSE, family = "cox", lower.limits = 0) ###
  ...
  # y_df_test = Surv(test_data$OS_Days_2Yrs, test_data$OS_Index_2Yrs) ###
  ...
  test_data3$binary_score = (test_data3$lasso_scores>=quantile(test_data3$lasso_scores,probs=0.5))
...
write.csv(goodModelGeneCounts,"goodModelCounts_CK_PFS_5Years_3celltypes_CW.csv")

goodModelGeneCounts = read.csv("goodModelCounts_CK_PFS_5Years_3celltypes_CW.csv",row.names = 1)
...
#final_genes = names(goodModelGeneProps)[which(goodModelGeneProps>0.5)]
final_genes = names(goodModelGeneProps)[which(goodModelGeneProps>0.05)]
final_genes = names(goodModelGeneProps)[which(goodModelGeneProps>0.2)]
...
model_cv <- glmnet(as.matrix(train_data3[,1:length(final_genes)]), y_df, alpha = 0, lambda = 0, standardize = FALSE, family = "cox", lower.limits = 0) ###
final_coeffs <- predict(model_cv, as.matrix(train_data3[,1:length(final_genes)]), type = "coefficients")  
final_coeffs = final_coeffs[,1]  

print(final_coeffs)
write.csv(as.matrix(final_coeffs),"final_coeffs_PFS_5Years_3celltypes_CW_CK_resistance.csv")
...
#coeffs = read.csv('final_coeffs_PFS_5Years_3celltypes_CK_resistance_0p1.csv')
coeffs = read.csv('final_coeffs_PFS_5Years_3celltypes_CK_resistance_0p1.csv')
coeffs1 = coeffs$V1
...
A = ggsurvplot(fit, data = train_data_2,
               ...
               legend.title = "sig scores (resistance)",               # Change legend titles
...
A$plot <- A$plot+
  ggplot2::annotate("text",
                    x = 30, y = 0.4, # x and y coordinates of the text
                    label = "HR = 5.1 (1.6-16) \n p = 0.002*** (Log-Rank-2-sided) \n cutpoint = median", size = 4)
A


## heatmap

# pdf(file = paste("output_split_CK_final.pdf", sep=""))
# 
# # Training set 
...
# pheatmap(train_data4,annotation_row = train_data5, main = "Signature genes CK Comp",
