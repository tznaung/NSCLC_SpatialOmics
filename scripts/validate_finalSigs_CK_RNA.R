...
patIds = rownames(data_CK)
nPat = length(patIds)

maxN_coeff = 8 # HR = 0.305, p = 0.255 
...
#y_df_test = Surv(CK$PFS_2Years_months, CK$PFS_2Years_index) ###
y_df_test = Surv(CK$OS_5Years_months, CK$OS_5Years_index) ###
CK$binary_score = (sigScores[,1]>=quantile(sigScores[,1],probs=0.5)) #p=0.07

cox <- coxph(y_df_test ~ binary_score, data = CK)
...
fit <- survfit(y_df_test ~ binary_score, data = CK)
fit$n.risk
# # Plot informative survival curves
A = ggsurvplot(fit, data = CK,
...
A$plot <- A$plot+
  ggplot2::annotate("text",
                    ...
                    label = "HR = 2.3 (0.79-6.8) \n p = 0.06 (Log-Rank-1-sided) \n cutpoint = median", size = 4)
A
